...
riot.tag2('cxb', '<div> <div class="t25"></div> <div class="t50"> <div class="cxdisplay"> <fla class=""></fla> <label class="mcode">&nbsp</label> <label class="mcode" onclick="{showGlyph}">&nbsp;{letr}</label> <label class="mcode gcode">&nbsp;{glyph}</label> <label class="mcode">&nbsp</label> </div> </div> <div class="buttonDiv"> <button each="{letters}" onclick="{keyed}" ontouchdown="{keyed}" class="{bright: bright, hide: hidden}" disabled="{disabled}"> <span>{letter.toUpperCase()}</span> </button> </div> <hr>', 'cxb fla,[data-is="cxb"] fla{ pointer-events: none; } cxb .mcode,[data-is="cxb"] .mcode{ font-weight: bolder; font-style: normal; color: black; } cxb .buttonDiv button,[data-is="cxb"] .buttonDiv button{ width: 2.2em; height: 1.5em; border-radius: 5em; margin: 0.1em; outline: none; } cxb .hide,[data-is="cxb"] .hide{ display: none; }', '', function(opts) {
